...
def covered_field_paths(nodes: list[dict], root_pattern: str) -> list[str]:
    """模式（含 X）匹配的根子树下全部 field 路径。"""
    fields: list[str] = []
    for rp in match_value_paths(nodes, root_pattern):
        root = find_value_node(nodes, rp)
        if root is None:
            continue
        for f in _walk([root]):
            if f["kind"] == "field":
...
def _normalize_children_from_template(
    ...
    if not tpl_children:
        return []
    raw_by_title = {
        rc.get("title"): rc for rc in raw.get("children") if isinstance(raw, dict) and isinstance(rc, dict) and rc.get("title")
