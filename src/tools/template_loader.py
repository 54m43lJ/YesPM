...
def load_template(path: Path | str) -> list[dict]:
    """加载并校验模板 YAML，返回归一化模板树（list[dict]，含推断的 kind）。"""
    path = Path(path)
    try:
    ...
    from .value_tree import compute_template_paths
...
def load_default_template() -> list[dict]:
    """加载内置默认 PRD 模板。"""
    return load_template(Path(__file__).resolve().parent.parent / "templates" / "default_prd.yaml")
