...
阶段内循环（① ↔ ①′）：`draft_prd` 每次提问收到回复后交由 `audit_input` 判定，未达标则带着具体反馈重新提问，达标才进入下一话题；单个章节讨论成熟后由 LLM 总结提炼写入取值树。
...
- **处理**: 以**对话方式**逐章节讨论细节，而非死板逐字段问答；讨论成熟后总结为章节
  - 按位置遍历模板，依字段有效 tier 决定讨论深度：
    - `P0` 逐字段深入对话讨论，必要时多轮追问
    - `P1` 按章节聚合，一次讨论一个章节的细节
    - `P2` 不主动提问，章节末尾由 LLM 基于上下文推断填入
  - 每轮：LLM 依 field 的 `question` / `example` 及讨论进展生成自然对话式提问 → `interrupt` 等待回复 → **先由 `audit_input` 判定达标**，达标后由 LLM 从讨论内容中结构化抽取值 → 经统一格式保障机制校验与用户预览确认后写入取值树（见 [结构化数据格式保障机制](#结构化数据格式保障机制全应用统一)）
  - 单个章节讨论成熟后（达标且无遗留话题），LLM 将多轮对话内容总结提炼为章节内容填入取值树
...
- **处理**: 独立的审核 agent（与 `draft_prd` 角色分离）判定本轮回复是否达到章节要求
  - 判定维度：信息充分性（是否覆盖本章节所需细节）、明确性（是否含糊或答非所问）、可执行性
  - 未达标时输出具体欠缺原因与追问建议，供 `draft_prd` 重新提问
- **输出**: `audit_result`（`{passed, feedback}`）+ `audit_passed`
- **路由**: 未达标 → 回到 ① 基于 feedback 重新提问；达标 → 继续下一话题或总结章节
...
- `draft_prd` → `audit_input`: 每次收到用户回复即进入阶段内审核
- `audit_input` → `draft_prd`: 未达标，基于 `feedback` 重新提问（阶段内循环，不计入 `iteration_count`）
- `audit_input` → （继续）: 达标，继续下一话题或总结章节；取值树填充完成后进入 ②
...
所有产生结构化数据的环节——P0/P1 用户输入解析、P2 推断、review 审核输出，以及未来的 `audit_input` / `polish_prd`——统一采用「**解析 → 确定性校验 → LLM 修复重试 → HITL 预览确认**」的保障链路。该机制是全应用级通用能力，**不归属于任何单个节点**。
